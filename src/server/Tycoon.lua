...
function Tycoon.new(player, spawnPoint)
	...
	self.Owner = player
	self._spawn = spawnPoint -- _ prefix means this variable is treated as private by convention
...
function Tycoon:Init()
	...
	self:LockAll()
	self:LoadUnlocks() -- before LockAll because we need to create tag "Unlockable" for unlockables
	self:WaitForRebith()
	self:WaitForExit()
end
...
-- Creates a component from a ModuleScript and runs its Init (here our modules working)
...
function Tycoon:WaitForRebith(player)
